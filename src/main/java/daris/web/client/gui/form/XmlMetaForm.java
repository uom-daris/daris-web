...
public class XmlMetaForm {
    ...
    private static boolean isHiddenDocAttribute(XmlAttribute attr) {
        return attr != null && attr.name() != null && ("id".equals(attr.name()) || "ns".equals(attr.name())
                || "tag".equals(attr.name()) || attr.name().startsWith("xmlns:"));
    }
    ...
    public static void addDocForEdit(Form form, XmlElement doc) {
        ...
        FieldGroup fg = new FieldGroup(
                new FieldDefinition(de.name(), de.type(), de.description(), null, de.minOccurs(), de.maxOccurs())) {
        ...
        if (attrs != null && !attrs.isEmpty()) {
            for (XmlAttribute attr : attrs) {
                ...
                Field<String> af = new Field<String>(
                        new FieldDefinition(attr.name(), ConstantType.DEFAULT, null, null, 0, 1)) {
                    @Override
        ...
        if (!doc.hasElements() && !doc.hasAttributes()) {
            ...
            if (as != null) {
                for (XmlAttribute a : as) {
                    ...
                    Field<String> af = new Field<String>(
                            new FieldDefinition(a.name(), StringType.DEFAULT, null, null, 0, 1));
    ...
    private static void addToFieldSetForView(FieldSet fs, XmlElement e) {
        ...
        if (e.hasAttributes()) {
            ...
            for (XmlAttribute a : e.attributes()) {
                Field<String> af = new Field<String>(
                        new FieldDefinition(a.name(), ConstantType.DEFAULT, null, null, 1, 1));
                af.setXmlType(XmlType.ATTRIBUTE);
                if (a.value() != null) {
                    af.setValue(a.value());
                }
