...
public class UserDescribe extends ObjectMessage<List<UserRef>> {
    ...
    protected void messageServiceArgs(XmlWriter w) {
        if (_domain != null) {
            if (_domain.authority() != null && _domain.authority().name() != null) {
                if (_domain.authority().protocol() != null) {
                    w.add("authority", new String[] { "protocol", _domain.authority().protocol() },
        ...
        if (ues != null && !ues.isEmpty()) {
            ...
            for (XmlElement ue : ues) {
                ...
                if (ue.value("@authority") != null) {
                    authority = new Authority(ue.value("@protocol"), ue.value("@authority"));
                }
